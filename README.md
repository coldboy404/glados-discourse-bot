...
在 Cloudflare Dashboard 的 `glados-bot` 中配置 `BOT_TOKEN` 跟 `ADMIN_ID`。

在 Cloudflare 修改 `BOT_TOKEN` 后无需再访问 `/setup`：部署生效后最多一分钟，Bot 会自动用新 Token 将已有 webhook 重新注册，并同步只保留 `/start` 命令。`ADMIN_ID` 修改随部署即时生效。根路径只返回运行状态，不会调用 Telegram API。
